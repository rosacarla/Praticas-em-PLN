# -*- coding: utf-8 -*-
"""S3-Similaridade-Lexica.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Zs8EbM55OGmTeHMAlQysnLfMRkvv9G_9

># <b>Similaridade Léxica</b>  
>Instituição: PUCPR  
...
Existe uma série de diferentes cálculos/medidas que indicam a similaridade léxica entre palavras, chamadas de *string-based*.
...
# Lista com o dicionário de palavras válidas
...
"""Reimplemente a função `verificar(p)` de modo que:
1.  Caso a palavra digitada não exista no dicionário, é sugerido que todas as outras tem uma distância de edição de valor 1 da palavra digitada.
> **DICA**: Para percorrer a lista de palavras, você pode utilizar o comando `for palavra in dicionario:`
2.  Ao invés de receber apenas uma palavra, a função receba uma sentença inteira, faça a tokenização dela e ofereça sugestões para todas as palavras que nao estão no dicionário (caso tenham distância de edição igual a 1)
...
### **N-grams**
O N-grams são basicamente um conjunto de caracteres/palavras coocorrentes em uma determinada janela de abertura. Usaremos para comparar tanto **CARACTERES** quanto **PALAVRAS**.
"""

# Não há necessidade de implementar uma função de n-grams, pois o NLTK já implementa
import nltk
from nltk.util import ngrams

# Necessário porque utilizaremos o tokenizador
nltk.download('punkt')
...
#### 4) Por fim, nosso assunto atual, *Similaridade léxica*
...
A distância de Jaccard é definida como o *tamanho da interseção dividida pelo tamanho da união de dois conjuntos*. Usaremos para comparar tanto **CARACTERES** quanto **PALAVRAS**.
...
# Não há necessidade de implementar uma função de jaccard, pois o NLTK já implementa
...
<p align="justify"> Para aumentar ainda mais a pontuação de Jaccard no exemplo dado, uma etapa de pré-processamento que poderia ser aplicada é a <b>normalização de palavras</b>. No exemplo fornecido, palavras como "programas" e "programar" têm sentido semelhante, mas não são reconhecidas como semelhantes porque são tratadas como diferentes devido à conjugação verbal. Aplicar a normalização, como a lematização ou a stemming, poderia ajudar a aumentar a interseção de palavras entre as duas sentenças.</p>

>RESPOSTA 2

<p align="justify"> Para calcular o valor de Jaccard entre as duas sentenças fornecidas poderia ser utilizado este cõdigo:
"""

s1 = 'Presidente responde a imprensa no Paraná'
s2 = 'Bolsonaro fala em Curitiba'

s1_set = set(nltk.word_tokenize(s1, language='portuguese'))
s2_set = set(nltk.word_tokenize(s2, language='portuguese'))

jaccard_similarity = 1 - nltk.jaccard_distance(s1_set, s2_set)
print("Valor de Jaccard entre as duas sentenças:", jaccard_similarity)

"""<p align="justify"> O código calcula a similaridade de Jaccard entre as duas sentenças, reflete a similaridade entre elas com base na interseção e na união de seus conjuntos de palavras tokenizadas.</p>  
<p align="justify">OBSERVAÇÃO: Se o valor de Jaccard entre as duas sentenças é 0.0, isso significa que não há interseção entre os conjuntos de palavras tokenizadas das sentenças. Isso ocorre porque não há palavras em comum entre as duas sentenças. Portanto, a similaridade de Jaccard entre essas sentenças é 0, indicando que elas são completamente diferentes uma da outra com base nas palavras que contêm.</p>
